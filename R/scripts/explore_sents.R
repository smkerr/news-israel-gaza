...
# load corpus
corpus <- readr::read_csv(here::here("data/vintages/corpus_sent_2023-12-02.csv")) # latest version 
corpus <- corpus |> 
...
corpus |> 
  ...
  filter(n > 1) # TODO: 3,626 sentences are duplicates; remove duplicate sentences

# date & load_date -------------------------------------------------------------
# missings
corpus |> 
  filter(is.na(load_date)) # none
...
parse_fail_ids <- corpus |> 
  ...
  pull(doc_id)
corpus |> 
  filter(doc_id %in% parse_fail_ids) |> 
  count(newspaper) # all belong to The Guardian
corpus |> 
...
df <- corpus |> 
  mutate(
    ...
    label = case_when(
      !str_detect(text, "palest\\w+|gaza\\w+|israel\\w+") ~ NA_character_, # NA if no mention of Palestine (+Gaza) or Israel
      str_detect(text, "palest\\w+|gaza\\w+") &
        str_detect(text, "israel\\w+") ~ NA_character_, # NA if both are mentioned
      str_detect(text, "palest\\w+|gaza\\w+") ~ "palestine/gaza",
      str_detect(text, "israel\\w+") ~ "israel",
      TRUE ~ NA
    )
  )
